...
public struct MockAuthClient: AuthClienting {
    ...
    public func register(firstName: String, lastName: String, username: String, email: String, phone: String, password: String) async throws {
        try await Task.sleep(nanoseconds: 400_000_000)
        if email.lowercased().contains("fail") { throw AuthUIError.operationFailed("Email already used.") }
    }

    public func requestPasswordReset(email: String) async throws {
        try await Task.sleep(nanoseconds: 300_000_000)
        if !email.contains("@") { throw AuthUIError.invalidInput("Invalid email.") }
    }
